...
- **Potential upgrades are listed too.** An item on a higher upgrade track can end up better than what a character wears even when it starts out worse, so a piece is now shown if it wins today *or* if its track ceiling beats theirs. The item level difference shows three ways: green with an upward arrow, red with a downward one, and yellow with a ring when it is level for level — nothing moves, so nothing points anywhere
- **The upgrade track is an argument of its own, shown alongside the item level rather than instead of it.** The blue arrow and the track name appear whenever the item's track beats the one worn in that slot, even on a piece that already wins on item level — a better track says the lead will keep growing, which the `+N` alone does not. It still appears in the original case as well: behind today, further later. What it no longer does is show up where the track is level with theirs, which was never news
- **An item on the same track as what they wear is no longer offered at all.** Two pieces of one track share a ceiling, so the only thing between them is how far each has been pushed — and that is a bill in upgrade currency, not an upgrade: the worn piece reaches the same place for the same price. A Champion 6/6 next to their Champion 1/6 used to report a healthy `+N` and mean nothing by it. The slot is dropped before it is scored rather than scored zero, since a swap that buys nothing does not deserve a line — and for rings, trinkets and the two weapon slots the *other* slot is still judged normally
- Tracks are told apart by rank rather than by identity: `GI.UPGRADE_TRACKS` holds a separate entry per upgrade step, so Champion 1/6 and Champion 6/6 are different tables that have to compare equal here
- A two-hander is judged against the better-tracked of the two hands. It takes both slots, so it has to beat the stronger one to be worth the swap, not the more convenient one
...
- **Everything the game loads moved into a `GearInventory/` subfolder** — the TOC, `Addons`, `Libs`, `Locales`, `Options` and `Textures`. WoW requires the addon folder to be named after its TOC, which the repository folder is not, so installing meant copying a hand-picked list of paths; it is now one folder copied whole. Documentation and art sources stay at the repository root, where the game will never see them. Paths inside the TOC are relative to it and did not change
- **`GetRaceAtlas` never existed.** `GI.RaceAtlas` asked for it first and fell back to building the atlas name by hand, so the fallback was doing all the work and the first branch was dead — quietly, since it was guarded. There is no engine call for this at all: `C_CreatureInfo.GetRaceInfo` carries only the race name, its file string and its ID, and Blizzard's own UI source never assembles a `raceicon` name anywhere
- Two atlas spellings exist and both were measured with `C_Texture.GetAtlasExists`: `raceicon128-<race>-<sex>` and `raceicon-<race>-<sex>`. A `raceicon64-` prefix, an underscore spelling and the bare token all came back false, so the list of candidates went from five guesses to two facts. The check itself moved from `GetAtlasInfo`, which builds a whole table per candidate only to be tested for nil, to `GetAtlasExists`, which answers a boolean
